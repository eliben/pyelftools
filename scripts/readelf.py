...
class ReadElf(object):
    ...
    """
    def __init__(self, file, output, args):
        """ file:
    ...
    def display_program_headers(self, show_heading=True):
        ...
        for segment in self.elffile.iter_segments():
            if self.elffile.elfclass == 32:
                self._emitline(fmt % (
                    describe_p_type(segment['p_type']),
    ...
    def display_section_headers(self, show_heading=True):
        ...
        #
        if self.elffile.elfclass == 32:
            hdr = '  [Nr] Name              Type            Addr     Off    Size   ES Flg Lk Inf Al'
            fmt = '  [%2u] %-17s %-15.15s %s %s %s %s %3s %2s %3s %2s'
        else:
            if self.args.wide:
                hdr = '  [Nr] Name              Type            Address          Off    Size   ES Flg Lk Inf Al'
                fmt = '  [%2u] %-17s %-15.15s %s %s %s %s %3s %2s %3s %2s'
            else:
                hdr = '  [Nr] Name              Type            Address          Offset\n    Size              EntSize           Flags    Link  Info  Align'
                fmt = '  [%2u] %-17.17s %-15.15s  %s  %s\n  %s  %s %3s      %2s   %3s     %s'
        ...
        if self.elffile['e_machine'] == 'EM_AARCH64':
            self._emit('D (mbind), ')
    ...
    def _dump_debug_info(self):
        ...
        if not self._dwarfinfo.has_debug_info:
            return
        self._emitline('\nContents of the %s section:' % self._dwarfinfo.debug_info_sec.name)
        self._emitline()
...
def main(stream=None):
    ...
    with open(args.file, 'rb') as file:
        try:
            readelf = ReadElf(file, stream or sys.stdout, args)
            if do_file_header:
