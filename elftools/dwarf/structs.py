...
from ..common.construct_utils import (
    SLEB128, EmbeddableStruct, Embed, CStringBytes, exclude_last_value
)
from .enums import *
...
class DWARFStructs(object):
    ...
    def _create_initial_length(self):
        ...
        self.Dwarf_initial_length = _InitialLengthAdapter(
                Struct(
                    'first' / self.Dwarf_uint32,
                    'second' / If(lambda ctxt: ctxt.first == 0xFFFFFFFF,
                        self.Dwarf_uint64
                    )
                )
            )
